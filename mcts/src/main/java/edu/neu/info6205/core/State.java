...
import edu.neu.info6205.adt.bqs.UnorderedIterator;
...
public interface State<G extends Game> {
     ...
     *
     * @return true if it is not possible to follow this State of the game by any
     *         other State.
     */
    boolean isTerminal();
     ...
     *
     * @return of(w) where w is the index of the winner if this State is a win or
     *         loss;
    ...
    default Iterator<Move<G>> moveIterator(int player) {
        ...
        if (moves == null)
            throw new RuntimeException("moves returned null");
        else
            return UnorderedIterator.createDeterministic(moves, random());
    }

    /**
     * Method to choose a move for the given player.
     * NOTE: depending on the field <code>random</code>, the choice may be
